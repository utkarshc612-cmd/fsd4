 ...
 */
function generateAIQuiz(topic, difficulty = 'easy', count = 10, questionTypes = ['mcq']) {
  const questions = [];
  const typesList = Array.isArray(questionTypes) ? questionTypes : ['mcq'];

  for (let i = 0; i < count; i++) {
    // Rotate through question types
    const qType = typesList[i % typesList.length];
    let question = {};

    if (qType === 'mcq') {
      question = {
        id: Math.random().toString(36).substr(2, 9),
        text: `${topic} - Multiple choice question ${i + 1}?`,
        type: 'mcq',
        difficulty,
      ...
      question = {
        id: Math.random().toString(36).substr(2, 9),
        text: `${topic} - Short answer question ${i + 1}? (Answer in 2-3 lines)`,
        type: 'short',
        difficulty,
        marks: difficulty === 'easy' ? 2 : difficulty === 'medium' ? 3 : 4,
        explanation: `Sample answer should cover key points of ${topic}`
      };
    } else if (qType === 'truefalse') {
      question = {
        id: Math.random().toString(36).substr(2, 9),
        text: `${topic} - Is this statement true or false? (Question ${i + 1})`,
        type: 'truefalse',
        difficulty,
        marks: difficulty === 'easy' ? 1 : 2,
        correctAnswer: Math.random() > 0.5,
        explanation: `This tests your conceptual understanding of ${topic}`
      };
    }
  ...
  return {
    ...
    questions,
    estimatedTime: Math.ceil(count * 2.5),
    instructions: `Answer all ${count} questions. Difficulty: ${difficulty}. Question types: ${typesList.join(', ')}`
  };
}
