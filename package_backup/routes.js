...
router.post('/teacher-requests', (req, res) => {
  const { name, email, school, phone, designation, subjects, experience, qualification, reason, submittedAt } = req.body;
  
  if (!name || !school || !email || !subjects || subjects.length === 0) {
    return res.status(400).json({ error: 'Name, school, email, and subjects are required' });
  }
...
// Admin: update teacher request status
router.put('/admin/teacher-request/:id', (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  const request = db.teacherRequests.find(r => r.id === id);
...
router.post('/auth/login', (req, res) => {
  try {
    ...
    console.log(`AUTH/LOGIN: password hash="${passwordHash}"`);
    const teacher = db.teachers.find(t => t.username === username && t.passwordHash === passwordHash);
    if (!teacher) {
      console.log(`AUTH/LOGIN: teacher not found or password mismatch for user "${username}"`);
      return res.status(401).json({ error: 'Invalid credentials' });
    }
    const token = generateId();
    db.sessions[token] = { teacherId: teacher.id, createdAt: Date.now() };
    console.log(`AUTH/LOGIN: success for user "${username}", token="${token}"`);
    res.json({ token, teacher: { id: teacher.id, username: teacher.username, name: teacher.name, school: teacher.school } });
  } catch (e) {
    console.error('AUTH/LOGIN: error', e && e.message);
    res.status(500).json({ error: 'Server error during login', message: e && e.message });
  }
});
